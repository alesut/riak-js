...
var repoBasePath = path.join(__dirname, '../../../..') + '/';
/**
...
HttpRequest.prototype.execute = function() {
  ...
  var caller = 'not_found';
  var callerParts;
  var stacktrace;

  stacktrace = (new Error('TAKING_TRACE')).stack.split('\n');
  if(meta.resource == 'mapred'){
  ...
  callerParts = caller.trim().split(' ');
  if(callerParts.length == 2) {
    caller = callerParts[1];
  }
  else if(callerParts.length == 3) {
    caller = callerParts[2];
  }
  caller = caller.replace(repoBasePath, '').replace(/\//g, '__').replace(/:/g, '-').replace(/\(|\)/g, '');
  meta.tr_cl = caller;
  if(meta.data){
    meta.tr_sz = meta.data.length;
  }
  ...
  var request = httpClient.request(meta, function(err, response) {
    ...
    if (meta._pool && normalstatus.indexOf(response.statusCode) == -1) {
      ...
      // close response to prevent stream going into paused mode
      // https://github.com/joyent/node/wiki/Api-changes-between-v0.8-and-v0.10
      // should fix issue https://github.com/mostlyserious/riak-js/issues/210
      response.resume();
      return callback(e);
    }
        ...
        if (meta.buckets == 'stream') {
          ...
          if (buffer.length > 0) {
            ...
            if (m && m.index) { // whole or contiguous JSON chunks
              ...
              try {
                var buckets = JSON.parse(head).buckets;
                if (buckets && buckets.length) meta._emitter.emit('buckets', buckets);
              } catch (err) {
                this.emit('error', err);
              }
            }
    ...
    response.on('end', function() {
      buffer = self.concatChunks(buffer);
      var encoding = self.meta._headers['content-encoding'];

      if(encoding == 'gzip'){
        delete meta._headers['content-encoding'];

        zlib.gunzip(buffer, function(e, buf){
          if(e){
            // fallback
            self.responseEnd(buffer, meta, callback);
            return;
          }

          self.responseEnd(buf, meta, callback);
        });
      }
      else{
        self.responseEnd(buffer, meta, callback);
...
/**
 * Handles a chunks of buffer
 *
 * @api private
 */
HttpRequest.prototype.concatChunks = function(buffer) {
  var bytesRead = buffer.reduce(function(p, c){
    return p + c.length;
  }, 0);
 ...
 * @api private
 */
HttpRequest.prototype.responseEnd = function(buffer, meta, callback) {
  buffer = this.handleRequestEnd(buffer);
  var err = null;

  if(buffer instanceof Error){
    err = this.handleError(buffer);
  }

  this.client.emit('riak.request.finish', this.event);
