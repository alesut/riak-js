...
HttpClient.prototype.save = function(bucket, key, data, options, callback) {
  ...
  var meta = new Meta(this._defaults, { method: method }, options, { bucket: bucket, key: key, callback: callback });
  meta.loadData(data);
  var self = this;

  if(options.forceGzipCommpression){
    zlib.gzip(meta.data, function(err, buffer){
      meta.data = buffer;
...
HttpClient.prototype.buckets = function(options, callback) {
  options.buckets = options.buckets || true;

  var Meta = _metaType(options);
  var meta = new Meta(this._defaults, options, { buckets: options.buckets, callback: callback }),
    callback = meta.callback;

  if(options.buckets === 'stream') {
    ...
    meta.callback = function(err, data, _meta) { callback(err, data && data.buckets, _meta) }
    this._execute(meta);
  }
}
...
HttpClient.prototype.getBucket = function(bucket, options, callback) {
  ...
  var meta = new Meta(this._defaults, options, { bucket: bucket, callback: callback }),
    callback = meta.callback;
  meta.callback = function(err, data, _meta) { callback(err, data && data.props, _meta) }
  this._execute(meta);
}
...
HttpClient.prototype.keys = function(bucket, options, callback) {
  options = options || {};
  options.keys = options && options.keys || 'keys';

  var Meta = _metaType(options);
  var meta = new Meta(this._defaults, options, { bucket: bucket, keys: options.keys, props: false, callback: callback }),
    callback = meta.callback;

  if(options.keys === 'stream') {
    meta.callback = function(err, data, _meta) {
      if (_meta) delete _meta._emitter;
      callback(err, data, _meta);
    };
    ...
    meta.callback = function(err, data, _meta) { callback(err, data && data.keys, _meta) }
    this._execute(meta);
  }
}
...
HttpClient.prototype.query = function(bucket, query, options, callback) {
  ...
  var skipURIEncode = options && options.skipURIEncode;
  ...
  var key = "index/" + field + ( typedIndex? ( "_" + (typeof value == 'number' ? 'int' : 'bin') ): "" ) + "/" +
    (skipURIEncode ? value : encodeURIComponent(value));

  if (end) key += "/" + (skipURIEncode ? end : encodeURIComponent(end));
